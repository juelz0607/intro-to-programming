# Write a program called name.rb that asks the user to type in their name
# and then prints out a greeting message with their name included.

# Modify name.rb again so that it first assks the user for their first 
# name, saves it into a variable, and then does the same for the last name.
# Then outputs their full name all at once. 

# Initiating Statement/Declaring Variables

puts "Stranger! May I have your first name?"
fname = gets.chomp

puts "And now your last name?"
lname = gets.chomp

name = fname.capitalize+" "+lname.capitalize

# Writing Expressions

puts "Hello " + name + "!"

# Add another section onto name.rb that prints the name of the user 10
# times. You must do without this explicitly writing the puts method 10
# times in a row.

10.times {puts name}